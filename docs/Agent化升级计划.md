...
>   **1.3 工具声明式迁移 🟡 试点完成（2/31：`get_my_courses` + `select_course`，一读一写）**；
...
> - **下一步（按顺序，可直接开工）**：
>   1. **批量迁移剩余工具到声明式**（1.3 后半）：先教师/管理员各挑 1 个（覆盖"非本人课程拒绝"与"审批"两类语义），
>      再按角色批量；每迁一批跑 `node .dsh/eval-p5-tools.cjs --only=<迁移的工具>` 对照选路，
>      注意**描述可以写得更明确，但参数 Schema 的"形状"必须与手写版一致**（见开发记录 (二十一) 坑 ②）；
...
>   2. `mvn -o -B test -pl edu-api -am` 应 **214 项**全绿（188 + M2 新增 26）；
...
| 1.3 工具改造 | **🟡 试点完成（2026-09-22）**：`get_my_courses`（READ_ONLY）+ `select_course`（DANGEROUS）已迁为声明式（`@Tool` + 项目侧 `@ToolMeta` 补展示名/风险等级），手写实现保留作对照（`registerOverride` 接管）。**其余 29 个待批量迁移**。⚠️ 迁移踩到的两个坑见开发记录 (二十一)：`MethodToolCallback.Builder` 不会从注解推导定义；单个对象参数会被框架**再套一层参数名**（静默改变模型要填的形状） | `tool/declarative/` |
