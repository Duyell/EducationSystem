...
> ### ⏸ 暂停状态（2026-09-20 第二轮，下次直接从这里开始）
>
> - **代码状态**：教务扩展 P5 + 本轮「政策文档 + 2 个缺陷修复」均已提交推送，`main` = 见 `git log`。
>   工作区除 `frontend/.vscode/`（未跟踪，待你决定是否提交）外干净。
> - **服务状态**：**Redis 已起**（跑测试需要它），后端 8080 与前端 5173 未起。
> - **本轮新增交付**：`docs/policies/`（**JW-01 ~ JW-10 共 10 份制度文件 + README 索引**）——
>   这是 M3 阶段 RAG 的**知识库语料**，每条规则标注了 ⚙（系统已校验）/【制度】（未实现），
>   文末附"与系统实现的关系"表。**审计入口：`docs/policies/README.md`**（含"需你重点审的 6 处"与关键常量对照表）。
> - **顺手修掉的两个真缺陷**：① AI 助手录入成绩绕过 `ScoreService`（`passed` 不写、总分精度不一致）；
>   ② `evaluate_teacher` 评分范围 1~100 与界面 5 星不一致。详见开发记录 **(十一)**。
> - **下一步三选一**（等你定）：
>   **① 输出护栏**（小、独立：模型偶发把工具调用写成正文流给用户，要做成"不展示 + 当真正调用继续走"）；
>   **② 学业预警通知**（小、教务向：登录弹预警，需先定阈值，见第五节 C 线）；
>   **③ M2 框架化迁移**（Spring AI + Redis 多轮记忆 + 会话管理）。
>   建议顺序：① → ② → ③（前两个独立可交付，③ 最长）。
> - **动手前**：先起 Redis（否则 `AgentRateLimiterTest`/`LoginInterceptorTest` 会连不上 6379 而红——
>   这个坑本轮又踩了一次），再 `mvn -o -B test -pl edu-api -am`（应 **154 项**全绿）→
>   若动到工具/提示词，跑 `node .dsh/eval-p5-tools.cjs --inventory-only`（39 项，秒级）并**记录 ROUTING 分数当基线**。
> - **本次工作记录**：`docs/开发记录.md` 第 **(十一)** 节（政策文档 + 两个缺陷 + 版本更正）与
>   第 **(十)** 节（P5 交付与评测）；简历口径见 `docs/简历项目描述.md`。
...
**C. 学业预警通知（小功能，教务方向；作者 2026-09-20 审计 JW-01 时提出）**
- 需求（用户原话口径）：**只做学业预警，不做留级/退学等自动化处理**。
  学生满足条件后，**登录教务系统时弹出一条通知**；具体事宜由**辅导员约谈**；
  即使达到退学等严重程度，也只是**由管理员修改学生状态**（人工决定）。
- 已具备的数据：未通过课程清单（`passed=0` 的成绩 + 课程学分）、已修学分与学分缺口、
  GPA、专业内排名 —— 全部可算，见 `CreditServiceImpl.auditGraduation` / `GpaServiceImpl`。
- 待确认（**动手前必须定**）：**触发口径与阈值**。建议"未通过课程学分**累计** ≥ **8 学分**"，
  也可选"未通过门数 ≥ N""GPA 低于某值""学分缺口 ≥ 某值"（见 JW-01 5.2 的候选表）。
- 实现要点（待评估）：① 预警判定放在哪里（Service 实时计算 vs 预警记录表）；
  ② 通知如何下发（登录响应带 `warnings` 字段，或首页调 `/academic-warning/my` 后弹窗）；
  ③ 是否需要"只提示一次/可标记已读"；④ 阈值做成配置（照 `gpa_rule` 的做法）。
- 文档口径已就位：`docs/policies/01-学籍管理规定.md` §5（v1.1 已按此改写）。

> 已完成（不必重做）：教务业务扩展 P1–P5 全部交付并验证（培养计划/绩点 → 排课 →
> 选课 → 考试 → Agent 工具与评测）。设计文档 §4.5 规划的 11 个工具已全部上线，
> 工具面从 21 个扩到 30 个。另有 **M3 的 RAG 语料**：`docs/policies/` 10 份制度文件（作者正在逐章审计）。
