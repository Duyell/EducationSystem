...
>   **Agent 能力已初步完整**；**M3（RAG，pgvector）已开工**（向量库接线 + 语料进 jar + 真机验证，见开发记录 (二十四)）；剩余增强项：1.1（拆 `edu-agent` 模块）、1.9（断线重连）、管理端成绩变更日志页面。
...
>   2. `mvn -o -B test -pl edu-api -am` 应 **280 项**全绿（2 项为外部依赖用例，默认跳过）；RAG 真机用例：`-Dtest=RagConfigTest "-Dai.rag.live=true"`（需 pgvector 容器）；
...
> - **本次工作记录**：`docs/开发记录.md` 第 **(二十四)**（M3 起步 · pgvector 接入）、**(二十三)**（33 工具全量声明式迁移 · Agent 能力初步完整）、**(二十二)**（AgentRuntime + 事件契约）、**(二十一)**（工具声明式迁移）、**(二十)**（前端会话侧栏 +
