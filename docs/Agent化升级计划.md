...
> 更新到 2026-09-20：Agent 主线（M1 阶段 0）＋**教务业务扩展 P1/P2/P3/P4/P5 均已完成**并推送 `origin/main`。
> **教务业务扩展这条线到此收官**（P1 培养计划/绩点 → P2 排课 → P3 选课 → P4 考试 → P5 Agent 工具与评测），
> 下一步回到 Agent 主线：**M2 框架化迁移（Spring AI + 多轮记忆）**。

> ### ⏸ 暂停状态（2026-09-20，下次直接从这里开始）
>
> - **代码状态**：P5 已提交并推送，`main` = `3c7bb34`。工作区除 `frontend/.vscode/`（未跟踪，待你决定是否提交）外干净。
> - **服务状态**：**全部已停**（Redis / 后端 8080 / 前端 5173）。开工先按第 1 节起 Redis + 后端；
>   要跑 Agent 评测再按第 1.1 节加 Ollama 环境变量。
> - **下一步二选一**（等你定）：**① 输出护栏**（小、独立：模型偶发把工具调用写成正文流给用户，
>   要做成"不展示 + 当真正调用继续走"）；**② M2 框架化迁移**（Spring AI + Redis 多轮记忆 + 会话管理）。
>   建议先 ① 再 ②，理由见第五节 B 线。
> - **动手前**：`mvn -o -B test -pl edu-api -am`（应 150 项全绿）→ 若动到工具/提示词，
>   跑 `node .dsh/eval-p5-tools.cjs --inventory-only`（39 项，秒级）并**记录 ROUTING 分数当基线**。
> - **本次工作记录**：`docs/开发记录.md` 第 (十) 节（P5 交付、三条真实缺陷、评测自身犯过的错、全绿数字）；
>   另有 `docs/简历项目描述.md`（简历条目，功能口径 + 被追问时的口径）。
> - **仍在等用户确认的事项**：见第 4 节（流式输出护栏、`sendSse()` 异常、`frontend/.vscode/`、
>   `views/ai/index.vue` 拆分、是否提供云模型 Key、是否补完整 4 年培养计划数据）。
> - **一个已知的、不影响使用的设计取舍**：`drop_course`（退课）目前是普通写操作、**不弹确认卡片**，
>   而选课/录成绩/开课申请/审批都弹。若认为退课也该确认，说一声即可改（一行风险等级的事）。
